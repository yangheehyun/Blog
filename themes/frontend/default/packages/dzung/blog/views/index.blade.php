...
<body>
<h2>Index page contain all post</h2>
Category:<br/>
@foreach($categories as $category)
    Category name: {{link_to('blog/category/'.$category->slug,$category->name, $attributes = array(), $secure = null)}}<br/>
    Number of post: {{$category->num_post}} <br/>
@endforeach
<hr/>
Tag:<br/>
@foreach($tags as $tag)
    Tag name: {{link_to('blog/tag/'.$tag->name,$tag->name, $attributes = array(), $secure = null)}}<br/>
